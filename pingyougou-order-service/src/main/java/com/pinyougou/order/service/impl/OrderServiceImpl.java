...
public class OrderServiceImpl implements OrderService {
	...
	public void add(TbOrder order) {
		...
		for (Cart cart : cartList) {
			...
			for (TbOrderItem orderItem : cart.getOrderItemList()) {
				...
				momey += orderItem.getTotalFee().doubleValue();
			}
			tbOrder.setPayment(new BigDecimal(momey));// 合计金额

			orderMapper.insert(tbOrder);
			// 将orderId加入到orderIdList中
			orderIdList.add(orderId + "");
			total_money += momey;// 总共的合计数
		}
		// 添加支付日志
		if ("1".equals(order.getPaymentType())) {// 如果是微信支付
			TbPayLog payLog = new TbPayLog();
			payLog.setOutTradeNo(idWorker.nextId() + "");// 支付订单号
			payLog.setCreateTime(new Date());// 支付日志的创建时间
	...
	public void updateOrderStatus(String out_trade_no, String transaction_id) {
		// 1.修改支付日志状态
		TbPayLog payLog = payLogMapper.selectByPrimaryKey(out_trade_no);// 根据订单号查询payLog
		...
		for (String orderId : orderIds) {
			...
			if (order != null) {
				...
				orderMapper.updateByPrimaryKey(order);//修改
			}
		}
		// 3.清除缓存中的支付日志对象
