...
public class UserServiceImpl implements UserService {
	...
	public PageResult findPage(TbUser user, int pageNum, int pageSize) {
		...
		@Autowired
		private Destination sendSmsDestination;
		...
		public void createSmsCode(final String phone) {
			...
			jmsTemplate.send(sendSmsDestination, new MessageCreator() {
				...
				public Message createMessage(Session session) throws JMSException {
					...
					Map map=new HashMap<>();
					map.put("code", code);
					message.setString("param", JSON.toJSONString(map));//参数
					return message;
				}
			});
		}
		...
		public boolean checkSmsCode(String code, String phone) {
			String redis_code = (String) redisTemplate.boundHashOps("code").get(phone);
			if(redis_code==null){
				return false;
			}
